...
import MetricsDashboard39 from '../partials/dashboard/MetricsDashboard39';
...
function DataMetrics5() {
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${isDarkMode ? 'bg-slate-100' : 'bg-custom'}`}>
          ...
          {/* <SignOutButton/> */}
            ...
            <div className="sm:flex p-2 sm:justify-between sm:items-center mb-2">
              ...
              <div className="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2 mt-5">
                ...
                <span><label className={`${isDarkMode ? 'text-white' : 'text-slate-600'}`} style={{fontWeight:"bold", fontSize:'20px'}}>JVM Statistics - GC </label> &nbsp;
             
  </span>   
  {/* <button 
        onClick={toggleDarkMode} 
        className={`relative inline-flex flex-shrink-0 h-6 w-12 cursor-pointer transition-colors ease-in-out duration-200 ${
          isDarkMode ? 'bg-indigo-600' : 'bg-gray-200'
        } rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500`}
      >
        <span className={`sr-only ${isDarkMode ? 'text-white' : 'text-gray-600'}`}>
        Light/Dark Mode
        </span>
        <span
          aria-hidden="true"
          className={`inline-block h-5 w-5 rounded-full transition ease-in-out duration-200 ${
  ...
  <div className="relative flex items-center">
    ...
    /></div>
            ...
            <div className="grid grid-cols-12 gap-3">
              ...
              <MetricsDashboard38  startDate={new Date(startDate).getTime()/1000} endDate={new Date(endDate).getTime()/1000} jobName={jobName}/>
              <MetricsDashboard39  startDate={new Date(startDate).getTime()/1000} endDate={new Date(endDate).getTime()/1000} jobName={jobName}/>
              <MetricsDashboard37  startDate={new Date(startDate).getTime()/1000} endDate={new Date(endDate).getTime()/1000} jobName={jobName}/>
          ...
          <footer className = {`p-2 md:px-4 md:py-4 z-10 left-20 bottom-0 w-full ${isDarkMode ? 'bg-white' : 'bg-custom3'}`}>
           <span style={{fontWeight:'600'}} className={`block text-sm ${isDarkMode ? 'text-black' : 'text-white'} sm:text-center dark:text-white ${isDarkMode ? 'bg-white' : 'bg-custom3'}`}>DAAS (Dashboard As A Service) ©  
