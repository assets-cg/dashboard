...
function ReleaseDashboard02(props) {
...
//   // const theme = useTheme();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const reqData = await fetch(`${API_URL}storypointscompletionofrelease?projectName=${localStorage.getItem('releaseProject')}`, {
          headers: {
    ...
    fetchData();
  }, [localStorage.getItem('releaseProject'), completed, assigned, percentage]);
...
<p className='text-lg font-medium text-center'>Story Points</p>
...
<hr className="w-px h-6 bg-slate-400 border-none" />
</div>
...
export default ReleaseDashboard02;
