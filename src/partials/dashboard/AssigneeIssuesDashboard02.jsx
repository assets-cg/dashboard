...
function AssigneeIssuesDashboard02() {
 ...
 useEffect( ()=>{
   const getEpicAssosiatedToAssignee= async()=>{
   const reqData= await fetch(`${API_URL}assigneeepics?project&boardId&AssigneeName=${name}`,{
    headers: {
    ...
    // <div className="col-span-full xl:col-span-12 bg-white shadow-lg rounded-sm border border-slate-200">
      <div className="col-span-full xl:col-span-12 bg-custom2 shadow-2xl rounded-3xl border-solid border-2 border-green-600 scrollTable3">
       <div className="px-5 pt-5">
        ...
        <h3 className="font-semibold text-black">Details Of Epic Associated to {name}</h3>    
          ...
          <table className="table-auto w-full" >
            ...
            <thead className="text-xs  text-black bg-slate-50 rounded-sm">
              <tr>
                ...
                <th className="p-2">
                  <div className="font-semibold text-center">Epic Key</div>
                </th>
                <th className="p-2">
            ...
            <tbody className="text-sm font-medium" >
              {
                ...
                <tr key={key}>
                  ...
                  <div className="flex items-center">
                    <div className="text-white">{epicDetails.epicId}</div>
                  </div>
                </td>
                <td className="p-2">
                  <div className="text-center text-white">{epicDetails.epicKey}</div>
                </td>
                <td className="p-2">
                  <div  style={{ border: '1px solid', borderRadius:'4px', color: epicDetails.epicStatus === "Done" ? "darkgreen" : epicDetails.epicStatus === "In Progress" ? "yellow" : epicDetails.epicStatus === "TO DO" ? "#ff7b00" :  "red" }}className="text-center">{epicDetails.epicStatus}</div>
                </td>
                <td className="p-2">
                  <div className="text-center text-white">{epicDetails.assignTo}</div>
                </td>
                </tr>
                )
              }
...
export default AssigneeIssuesDashboard02;