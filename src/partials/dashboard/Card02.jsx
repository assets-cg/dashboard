...
function Card02(props) {
 ...
 useEffect( ()=>{
   ...
   const reqData= await fetch(`${API_URL}sprint_featureassociated?project=&boardID=&sprintId=1`,{
    headers: {
  ...
  return (
    <div className="col-span-full xl:col-span-7 bg-white shadow-2xl rounded-3xl border border-slate-200 ">
      <header className="px-5 py-4 border-b border-slate-100">
        <h3 className="font-semibold text-slate-800">Features Associated to Sprint</h3>
      </header>
      <div className="p-3">
        {/* Table */}
          ...
          <table className="table-auto w-full">
            ...
            <thead className="text-xs uppercase text-slate-400 bg-slate-50 rounded-sm">
              <tr>
                ...
                <th className="p-2">
                  <div className="font-semibold text-center">FeatureKey</div>
                </th>
                <th className="p-2">
            ...
            <tbody className="text-sm font-medium divide-y divide-slate-100">
              {
                ...
                <tr key={key}>
                  ...
                  <div className="flex items-center">
                    <div className="text-slate-800">{featureDetails.id}</div>
                  </div>
                </td>
                <td className="p-2">
                  <div className="text-center">{featureDetails.featureKey}</div>
                </td>
                <td className="p-2">
                  <div className="text-center text-green-500">{featureDetails.status}</div>
                </td>
                <td className="p-2">
                  <div className="text-center">{featureDetails.summary}</div>
                </td>
                <td className="p-2">
                  <div className="text-center text-sky-500">{featureDetails.assignedTo}</div>
                </td>
                </tr>
                )
              }
