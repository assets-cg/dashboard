...
const MetricsDashboard46 = ({ startDate, endDate }) => {
 ...
 const novu = new Novu(`${Novu_Api}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const sCom=[];
      const result = await fetch(`https://${localStorage.getItem("PromUrl")}:9090/api/v1/query?query=system_load_average_1m{job="${jobName}"}[5d]`);
      // console.log(result)
  ...
  .sort((a, b) => new Date(a.time) - new Date(b.time)); // sort by date ascending
     ...
     for(let i=0; i<lab.length; i++){
      if(`Load Average` === lab[i].graphName){
  ...
  return (
    <div className="col-span-full xl:col-span-12 bg-custom2 shadow-2xl p-5 h-80 rounded-3xl border-solid border-2 border-green-600  scrollTable4 ">
      <div><h2 style={{fontWeight:'600', color:'black'}}>Load Average</h2></div>
...
export default MetricsDashboard46;
