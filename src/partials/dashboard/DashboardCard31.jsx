...
//    const sPercentage=[];
   ...
   const reqData= await fetch(`${API_URL}taskstatuswise?projectName=${localStorage.getItem("projectName")}&BoardId=${localStorage.getItem("boardId")}`,{
    headers: {
   ...
   for(var i=0;i<resData.details.length;i++){
    sEpicId.push(resData.details[i].status);
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-6 bg-custom2 shadow-2xl border-solid border-2 border-green-600 rounded-3xl">
      ...
      <div className="flex items-center">
        <h2 className=" font-semibold text-black mr-2">Status wise Task Count</h2>
  </div>
      </header>
