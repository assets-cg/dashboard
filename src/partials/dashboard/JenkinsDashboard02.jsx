...
//    const sPercentage=[];
   const sEnv=[];
   const sCount=[];
   const getFailedDeploymentFrequency= async()=>{
   const reqData= await fetch(`${API_URL}faileddeployementfrequency?Environment=${envi}&days=${selectedOption}`,{
    headers: {
...
//  console.log(sTickets[0])
   ...
   getFailedDeploymentFrequency();
},[envi, selectedOption]);



  return (
    <div className="flex flex-col col-span-full sm:col-span-6 xl:col-span-6 border-solid border-2 border-green-600  h-80 bg-custom2 shadow-2xl rounded-3xl" >
...
Selected {env} Env ↓ </button>
      <Menu
        ...
        onClose={handleClose}
      >
        <MenuItem component={Link} className={classes.link} onClick={() => setEnvi("QA")} >
          QA
        </MenuItem>
        <MenuItem component={Link} className={classes.link} onClick={() => setEnvi("PROD")}>
          PROD
        </MenuItem>
        <MenuItem component={Link} className={classes.link} onClick={() => setEnvi("DEV")}>
          DEV 
        </MenuItem>
      </Menu>  */}
      <h2 className="text-lg font-semibold text-black mb-2">Failed Deployment Job Frequency on {envi}</h2>
...
export default JenkinsDashboard02;
