...
const MetricsDashboard35 = ({startDate, endDate}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const sCom2=[];
      const sCom3=[];
      const result = await fetch(`https://${localStorage.getItem("PromUrl")}:9090/api/v1/query?query=jvm_threads_daemon_threads{job="${jobName}"}[5d]`);
      const result2 = await fetch(`https://${localStorage.getItem("PromUrl")}:9090/api/v1/query?query=jvm_threads_live_threads{job="${jobName}"}[5d]`);
      const result3 = await fetch(`https://${localStorage.getItem("PromUrl")}:9090/api/v1/query?query=jvm_threads_peak_threads{job="${jobName}"}[5d]`);
      // console.log(result)
  ...
  return (
    <div className="col-span-full xl:col-span-7 bg-custom2 shadow-2xl border-solid border-2 border-green-600 p-6 h-80 rounded-3xl ">
      <div><h2 style={{fontWeight:'600', color:'black'}}>Thread Counts</h2></div>
       <Line data={chartData} options={chartOptions} height={350} />  
         </div>     );
};
export default MetricsDashboard35;