...
//   const [avg , setAvg] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const sCom=[];
      const result = await fetch(`https://${localStorage.getItem("PromUrl")}:9090/api/v1/query?query=jvm_memory_used_bytes{job="${jobName}", area="heap"}[1h]`);
      const result2 = await fetch(`https://${localStorage.getItem("PromUrl")}:9090/api/v1/query?query=jvm_memory_max_bytes{job="${jobName}", area="heap"}[1h]`)
...
//   console.log(avg)
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-5 xl:col-span-4 bg-custom2 shadow-2xl border-solid border-2 border-green-600 rounded-3xl">
      <div title='% of Heap used by the Container: API(s)' className="px-5 pt-5 app-timing">
        <header className="flex justify-between items-start mb-1">
        </header>
        <h2 style={{textAlign:'center', fontSize:'15px'}} className="text-lg font-semibold text-slate-800 mb-1">Container: API(s) Heap Used</h2>
        ...
        width={250}
        value={heap}
        currentValueText={heap  + "%  (Heap Used)"}
        needleColor="black"
        startColor="green"
        segments={10}
        endColor="red"
...
export default MetricsDashboard48;
