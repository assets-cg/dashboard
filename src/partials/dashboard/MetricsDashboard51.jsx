...
const MetricsDashboard51 = ({ startDate, endDate}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const sCom2=[];
      const sCom3=[];
      const result = await fetch(`https://${localStorage.getItem("PromUrl")}:9090/api/v1/query?query=hikaricp_connections_active{job="${jobName}", pool="HikariPool-1"}[5d]`);
      const result2 = await fetch(`https://${localStorage.getItem("PromUrl")}:9090/api/v1/query?query=hikaricp_connections_idle{job="${jobName}", pool="HikariPool-1"}[5d]`);
      const result3 = await fetch(`https://${localStorage.getItem("PromUrl")}:9090/api/v1/query?query=hikaricp_connections_pending{job="${jobName}", pool="HikariPool-1"}[5d]`);
      // console.log(result)
  ...
  return (
    <div className="col-span-full xl:col-span-9 bg-custom2 shadow-2xl border-solid border-2 border-green-600  p-5 h-60 rounded-3xl ">
      <div><h2 style={{fontWeight:'600', color:'black'}}>Connection Counts</h2></div>
       <Line data={chartData} options={chartOptions} height={260} />  
         </div>     );
};
export default MetricsDashboard51;